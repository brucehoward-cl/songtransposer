...
class Dominant7Chord(IChord):
    ...
    def __init__(self, name):
        ...
        index_for_seventh = SCALE.index(self.root) + 11
        if (index_for_seventh) > 11:
            index_for_seventh -= 12
        self.seventh = SCALE[index_for_seventh]

        print(f'(from Chord {self.name}); \troot: {self.root}; \tthird: {self.third}; \tfifth: {self.fifth}; \tseventh: {self.seventh}')
...
#        super().transpose(number_of_halfsteps)
        ...
        self.third = self.convert_note(self.third, number_of_halfsteps)
        self.fifth = self.convert_note(self.fifth, number_of_halfsteps)
        self.seventh = self.convert_note(self.seventh,number_of_halfsteps)
        self.name = f'{self.root}7'
